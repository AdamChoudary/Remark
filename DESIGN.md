...
Purpose: establish authority by naming exactly what's built. Six capabilities (Web Development, Voice Agents, Chat Bots, CRM & ERP, Analytics & Insights, Scalable & Secure — from `CONTENT.md`) as numbered editorial rows, hairline rule between each, no cards, no equal-width columns. Rows share a fixed collapsed height *(corrected 2026-07-02 — the row-height variation this line used to claim never existed in code; the real rhythm mechanism is each row's accordion expansion, which genuinely changes height on interaction rather than faking static variance)*. Mark word: **Voice**, red drawn circle (distinct gesture from Hero's underline) on row 02 "AI Voice Agents." Register: void (dark), alternating back from Narrative's paper. Eyebrow: none. Scroll-tied SVG (added 2026-07-02, same mechanism as Narrative): `AccentGlow` drifts with section-scroll progress; `EdgeGeometry` draws in beside the "What we build" heading itself, not the section's raw top edge — the fixed header covers roughly the top 140px of viewport at all times, so anything anchored to a tall section's literal top-0 draws in while already scrolled out of view. Anchor scroll-tied marks to a real visible element's own position, the same lesson as Narrative's solution reveal.
...
### Seams (added 2026-07-02)
Section boundaries are sewn, not cut. No full-width hairline `border-y` strips between sections — a drawn line at a seam announces division, which is the opposite of flow. At real register changes (dark→paper, paper→dark, testimonial→contact) a **SeamStitch** crosses the boundary: a short vertical crimson thread, half in each section, drawn downward on scroll — the brand's mark used structurally. Only at register changes; a stitch between two already-continuous dark sections would be decoration. The StatsBar strip (centered 150+/300+/24-7 trio with border-y) was removed 2026-07-02: it was the banned hero-metric template, duplicated Testimonial's metrics, and existed only as a divider band. TrustStrip lost its borders and reads as the hero's closing caption on the same void surface. Testimonial and Contact share compressed adjacent padding so proof flows directly into action.
...
- **2026-07-02 (flow pass):** Division-feel fix across the whole landing page: SeamStitch motif at register boundaries, all inter-section `border-y` strips removed, StatsBar deleted (redundant metric band), TrustStrip dissolved into the hero's register, Testimonial→Contact padding compressed into one proof→action movement. `image-prompts.txt` added at project root for remaining imagery.
