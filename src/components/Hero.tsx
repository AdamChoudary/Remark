...
export function Hero() {
  ...
  const renderContent = (isOverlay = false) => (
    <div aria-hidden={isOverlay} className="flex flex-col gap-4 md:gap-2 py-6 -my-6">
      ...
      <div className="flex w-full flex-col items-start gap-6 md:flex-row md:items-center md:justify-between md:gap-10">
        <span 
          className={`block animate-fade-in-up text-[clamp(4rem,9vw,8rem)] font-normal leading-[0.8] tracking-normal ${isOverlay ? "text-accent brightness-120 drop-shadow-[0_0_2px_var(--color-accent)]" : "text-fg"}`} 
          style={{ fontFamily: 'var(--font-betha), sans-serif', animationDelay: "0.1s" }}
        >
          Creative
        </span>
        
        <div className={`animate-fade-in-up md:-translate-y-8 lg:-translate-y-10 ${isOverlay ? "invisible" : ""}`} style={{ animationDelay: "0.4s" }}>
      ...
      <span 
        className={`block animate-fade-in-up text-[clamp(4rem,9vw,8rem)] font-normal leading-[0.8] tracking-normal ml-2 md:ml-32 lg:ml-40 ${isOverlay ? "text-accent brightness-110 drop-shadow-[0_0_2px_var(--color-accent)]" : "text-fg"}`} 
    ...
    <section className="relative flex h-[100dvh] w-full flex-col justify-end overflow-hidden bg-void text-left">
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-6 pb-24 pt-32 md:px-8 md:pb-32 lg:pb-40">
        ...
        >
          ...
          <div 
            ref={overlayRef}
            className="absolute inset-0 py-6 z-20 pointer-events-none select-none opacity-0 transition-opacity duration-300"
            style={{
